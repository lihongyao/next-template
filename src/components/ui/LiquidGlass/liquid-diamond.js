(function () {
  ...
  function createShader(width, height, options) {
    ...
    function render() {
      ...
      feDisplacementMap.setAttribute('scale', String((maxScale * 2) / CANVAS_DPI));
      debugLabel.textContent = 'Displacement Map (scale = ' + maxScale.toFixed(2) + ')';
    }
    ...
    return {
      ...
      setStyle: function (style) {
        shell.style.backdropFilter =
          'url(#' + id + '-filter) contrast(1.08) brightness(1.2) saturate(1.08)';
  ...
  function attachLiquidPyramid(options) {
    ...
    function onPointerMove(event) {
      ...
      const b = bounds();
      x = elasticPosition(dragOriginX + (event.clientX - dragStartX), b.minX, b.maxX);
      y = elasticPosition(dragOriginY + (event.clientY - dragStartY), b.minY, b.maxY);
    ...
    return {
      ...
      setDebug: function (enabled) {
        shader.element.querySelector('canvas').style.display = enabled ? 'inline-block' : 'none';
