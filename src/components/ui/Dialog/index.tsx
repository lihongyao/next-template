...
const DialogComponent = forwardRef<DialogRef, DialogProps>((props, ref) => {
  ...
  useEffect(() => {
    ...
    autoDestroyTimer.current = setTimeout(() => {
      closeReasonRef.current = 'autoDestroy';
      setIsExiting(true);
    }, autoDestroy * 1000);
    return () => {
  ...
  useEffect(() => {
    ...
    const handlePopstate = () => {
      closeReasonRef.current = 'popstate';
      setIsExiting(true);
    };
...
export type DialogInstance<K extends DialogType = DialogType> = {
  ...
  _onAfterClose?: (reason: DialogCloseReason) => void;
  /** 内部字段：动画结束 promise */
  _afterClosePromise?: Promise<void>;
...
export const DialogProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateDialogs = useCallback(
    (updater: (prev: DialogInstance<DialogType>[]) => DialogInstance<DialogType>[]) => {
      setDialogs((prev) => {
        ...
        prev.map((d) => {
          ...
          const nextProps =
            typeof updater === 'function'
              ? (updater as (p: PropsOf<typeof type>) => void)(prevProps)
